...
// DeleteBucketAndLiveChannel 删除channel name和bucket，该函数为了简化sample，让sample代码更明了
func DeleteBucketAndLiveChannel(bucketName string) error {
	// New Client
	client, err := oss.New(endpoint, accessID, accessKey)
	if err != nil {
		return err
...
// DeleteBucketAndObject deletes the test bucket and its objects
func DeleteBucketAndObject(bucketName string) error {
	// New client
	client, err := oss.New(endpoint, accessID, accessKey)
	if err != nil {
		return err
